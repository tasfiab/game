...
func _process(delta: float) -> void:
	...
	if Input.is_action_just_pressed("interact"):
		if can_click_cake:
			_choosing_ingredients('cake', Global.dough_type, 0)
		
		elif can_click_bread:
			_choosing_ingredients('bread',Global.dough_type,0)
		
		elif can_click_vanilla:
			_choosing_ingredients('vanilla', Global.flavour, 1)
			
		elif can_click_chocolate:
			_choosing_ingredients('chocolate', Global.flavour, 1)
			
		elif can_click_strawberry:
			_choosing_ingredients('strawberry', Global.flavour_2, 2)
		
		elif can_click_lemon:
			_choosing_ingredients('lemon', Global.flavour_2, 2)
...
# Looks at dough dictionary and finds what dough is formed from the chosen ingredients
	if not Global.chosen_ingredients.has(""):
		...
		$dough.texture = (Global.dough_sprites[dough_formed])
		Global.dough_formed = true
		


func _choosing_ingredients(ingredient, type, index):
	type = ingredient
	Global.chosen_ingredients[index] = type
